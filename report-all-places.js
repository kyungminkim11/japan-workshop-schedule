(function workshopAllPlaceReports(){
  ...
  function reportStore(){
    return state?.shopping?.reportProjects&&typeof state.shopping.reportProjects==='object'?state.shopping.reportProjects:{};
  }
  ...
  function ensureStyle(){
    ...
    style.textContent=`
      ...
      .all-place-report-intro strong{font-size:15px;color:#51443a}
      .all-place-report-intro span{color:#7d6e63;font-size:12px;line-height:1.55}
      ...
      .report-project-card .report-place-title{display:flex;align-items:center;gap:8px}
      .report-project-card .report-place-title strong{min-width:0;font-size:16px;line-height:1.35}
      ...
      .report-field.primary-comment-field>span{font-size:16px;color:#9e5f45}
      .report-field.primary-comment-field textarea{min-height:190px;border-color:#ead2c5;background:#fff!important}
  ...
  function renderAllPlaces(){
    ...
    try{
      ...
      const signature=items.map(item=>{
        ...
        return`${item.id}:${statusOf(report)}:${report.updatedAt||''}`;
      }).join('|');
      if(root.dataset.allPlacesSignature===signature&&root.dataset.allPlacesReady==='1'){
        enhanceDialog();
        return;
      }
      ...
      DAYS.forEach(day=>{
        ...
        dayItems.forEach(item=>{
          ...
          card.innerHTML=`
            <span class="report-card-top"><span>${safe(item.time)||'시간 미정'}</span><span class="report-status">${STATUS[status].icon} ${STATUS[status].label}</span></span>
            <span class="report-place-title"><span class="report-place-type">${TYPE_ICON[item.type]||'📌'}</span><strong>${safe(item.title)||safe(item.place)}</strong></span>
            <small>${safe(item.place)||'장소 정보 없음'}</small>
            <span class="report-progress"><i style="width:${progress.percent}%"></i></span>
            <span class="report-progress-label">${progress.filled}/${progress.total} 항목 작성</span>
            <span class="report-comment-hint">장소 코멘트 작성 ›</span>`;
  ...
  function enhanceDialog(){
    ...
    if(field){
      ...
      if(label&&label.textContent!=='장소별 코멘트')label.textContent='장소별 코멘트';
    ...
    if(fields&&!fields.querySelector('.report-optional-label')){
      ...
      const first=fields.querySelector('.report-field');
      first?.insertAdjacentElement('afterend',label);
    }
    const title=dialog.querySelector('#reportDialogTitle');
    if(title&&activeItemId){
      const item=allItems().find(entry=>entry.id===activeItemId);
      if(item)title.textContent=item.title||item.place;
    }
  }
  ...
  function boot(){
    if(typeof state!=='object'||!document.getElementById('appPanel')){
      setTimeout(boot,80);
      return;
    }
    ensureStyle();
    ...
    window.addEventListener('workshop-report-list-updated',()=>setTimeout(renderAllPlaces,20),{passive:true});
