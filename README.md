...
|---|---|---|---|
| 관리자 | 경민님 | 전체 일정, 메모, 사진, 위치, 접속 기록, 코드 관리 | 가능 |
...
3. 관리자 메모, 회사 업무 메모, 구매 체크리스트, 일정 수정 UI가 보이지 않는지 확인합니다.

### 가족 모드

1. 로그아웃 후 가족 접속 코드로 접속합니다.
2. 일정, 공유 상태, 체크인, 가족 공유 사진이 보이는지 확인합니다.
3. 여자친구 부탁 리스트와 관리자용 상세 메모가 보이지 않는지 확인합니다.
...
- 관리자 모드만 `notes`, `shoppingItems`, 접속 코드, 접속 기록을 볼 수 있습니다.
